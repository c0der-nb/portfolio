...
app.listen(PORT, () => {
  ...
  if (process.env.GMAIL_APP_PASSWORD) {
    ...
    gmailService.testConnection()
      .then(result => {
        console.log(`📧 Gmail connection: ${result ? '✅ Connected' : '❌ Failed'}`);
      })
      .catch(error => {
        console.error('❌ Gmail connection test failed:', error.message);
      });
  } else {
    console.log('⚠️  Gmail app password not configured. Please set GMAIL_APP_PASSWORD in your environment variables.');
  }
});
